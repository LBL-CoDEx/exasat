...
void smooth(level_type * level, int phi_id, int rhs_id, double a, double b){
  ...
  for(s=0;s<2*NUM_SMOOTHS;s++){ // there are two sweeps per GSRB smooth
    ...
    for(block=0;block<level->num_my_blocks;block++){
      ...
      double *  phi      = level->my_boxes[box].vectors[       phi_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
            double *  phi_new  = level->my_boxes[box].vectors[       phi_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
      ...
      const double *  valid    = level->my_boxes[box].vectors[VECTOR_VALID ] + ghosts*(1+jStride+kStride); // cell is inside the domain
      const double *  RedBlack[2] = {level->RedBlack_FP[0] + ghosts*(1+jStride), 
      ...
      for(i=ilo;i<ihi;i++){
            ...
            phi[ijk] = phi[ijk] + RedBlack[EvenOdd][ij]*lambda*(rhs[ijk]-Ax); // compiler seems to get confused unless there are disjoint read/write pointers
      }}}
