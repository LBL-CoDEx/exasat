...
void residual(level_type * level, int res_id, int x_id, int rhs_id, double a, double b){
  ...
  for(block=0;block<level->num_my_blocks;block++){
    ...
    const double *  x      = level->my_boxes[box].vectors[         x_id] + ghosts*(1+jStride+kStride); // i.e. [0] = first non ghost zone point
    ...
    const double *  valid  = level->my_boxes[box].vectors[VECTOR_VALID ] + ghosts*(1+jStride+kStride); // cell is inside the domain
          double *  res    = level->my_boxes[box].vectors[       res_id] + ghosts*(1+jStride+kStride);
