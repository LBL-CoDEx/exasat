...
contains
  ...
  subroutine advance (lo,hi,dt,a,b,Unew,U,difflux,flux)

    integer,          intent(in ) :: lo(3),hi(3)
    double precision, intent(in ) :: dt,a,b
    double precision, intent(out) ::    Unew(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),5)
    double precision, intent(in ) ::       U(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),5)
    double precision, intent(in ) :: difflux(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),5)
    double precision, intent(in ) ::    flux(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),5)
    ...
    do k = lo(3),hi(3)
       do j = lo(2),hi(2)
          do i = lo(1),hi(1)
             Unew(i,j,k,irho) = a * U(i,j,k,irho) + b * (Unew(i,j,k,irho) + &
                                dt * (difflux(i,j,k,irho) + flux(i,j,k,irho)))
             Unew(i,j,k,imx)  = a * U(i,j,k,imx) + b * (Unew(i,j,k,imx) + &
                                dt * (difflux(i,j,k,imx) + flux(i,j,k,imx)))
             Unew(i,j,k,imy)  = a * U(i,j,k,imy) + b * (Unew(i,j,k,imy) + &
                                dt * (difflux(i,j,k,imy) + flux(i,j,k,imy)))
             Unew(i,j,k,imz)  = a * U(i,j,k,imz) + b * (Unew(i,j,k,imz) + &
                                dt * (difflux(i,j,k,imz) + flux(i,j,k,imz)))
             Unew(i,j,k,iene) = a * U(i,j,k,iene) + b * (Unew(i,j,k,iene) + &
                                dt * (difflux(i,j,k,iene) + flux(i,j,k,iene)))
          end do
       end do
    end do
    !$OMP END PARALLEL DO
  ...
  subroutine hypterm (lo,hi,ng,dx,cons,q,flux)
    ...
    double precision :: fluxirho, fluximx, fluximy, fluximz, fluxiene 
