...
!EXPAND                     first_deriv_8( U(i-4:i+4,j,k,n)*un )
                Fhyp(i,j,k,n) = Fhyp(i,j,k,n) - dxinv(1) * &
                   ( D8(1)*(U(i+1,j,k,n)*Q(i+1,j,k,qu)-U(i-1,j,k,n)*Q(i-1,j,k,qu)) &
                   + D8(2)*(U(i+2,j,k,n)*Q(i+2,j,k,qu)-U(i-2,j,k,n)*Q(i-2,j,k,qu)) &
                   + D8(3)*(U(i+3,j,k,n)*Q(i+3,j,k,qu)-U(i-3,j,k,n)*Q(i-3,j,k,qu)) &
                   + D8(4)*(U(i+4,j,k,n)*Q(i+4,j,k,qu)-U(i-4,j,k,n)*Q(i-4,j,k,qu)) )
             end do
...
!EXPAND                     first_deriv_8( U(i,j-4:j+4,k,n)*un )
                Fhyp(i,j,k,n) = Fhyp(i,j,k,n) - dxinv(2) * &
                   ( D8(1)*(U(i,j+1,k,n)*Q(i,j+1,k,qv)-U(i,j-1,k,n)*Q(i,j-1,k,qv)) &
                   + D8(2)*(U(i,j+2,k,n)*Q(i,j+2,k,qv)-U(i,j-2,k,n)*Q(i,j-2,k,qv)) &
                   + D8(3)*(U(i,j+3,k,n)*Q(i,j+3,k,qv)-U(i,j-3,k,n)*Q(i,j-3,k,qv)) &
                   + D8(4)*(U(i,j+4,k,n)*Q(i,j+4,k,qv)-U(i,j-4,k,n)*Q(i,j-4,k,qv)) )
             end do
...
!EXPAND                     first_deriv_8(U(i,j,k-4:k+4,n)*qwUN)
                Fhyp(i,j,k,n) = Fhyp(i,j,k,n) - dxinv(3) * &
                   ( D8(1)*(U(i,j,k+1,n)*Q(i,j,k+1,qw)-U(i,j,k-1,n)*Q(i,j,k-1,qw)) &
                   + D8(2)*(U(i,j,k+2,n)*Q(i,j,k+2,qw)-U(i,j,k-2,n)*Q(i,j,k-2,qw)) &
                   + D8(3)*(U(i,j,k+3,n)*Q(i,j,k+3,qw)-U(i,j,k-3,n)*Q(i,j,k-3,qw)) &
                   + D8(4)*(U(i,j,k+4,n)*Q(i,j,k+4,qw)-U(i,j,k-4,n)*Q(i,j,k-4,qw)) )
             end do
...
!EXPAND                     +    dot_product(dxe(i-4:i+3,j,k,n), M8X)
    ...
    do n=2,ncons
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                Fdif(i,j,k,n) = Fdif(i,j,k,n) + (Hg(i+1,j,k,n) - Hg(i,j,k,n)) * dx2inv(1)
             end do
          end do
       end do
       !$omp end do nowait
...
!EXPAND                     +    dot_product(dxe(i,j-4:j+3,k,n), M8X)
    ...
    do n=2,ncons
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                Fdif(i,j,k,n) = Fdif(i,j,k,n) + (Hg(i,j+1,k,n) - Hg(i,j,k,n)) * dx2inv(2)
             end do
          end do
       end do
       !$omp end do nowait
...
!EXPAND                     +    dot_product(dxe(i,j,k-4:k+3,n), M8X)
    ...
    do n=2,ncons
       ...
       do k=lo(3),hi(3)
          do j=lo(2),hi(2)
             do i=lo(1),hi(1)
                Fdif(i,j,k,n) = Fdif(i,j,k,n) + (Hg(i,j,k+1,n) - Hg(i,j,k,n)) * dx2inv(3)
             end do
          end do
       end do
       !$omp end do nowait
    ...
    do m = 1, ncons
       ...
       do k = lo(3),hi(3)
          do j = lo(2),hi(2)
             do i = lo(1),hi(1)
                Uprime(i,j,k,m) = Fhyp(i,j,k,m) + Fdif(i,j,k,m)
             end do
          end do
       end do
          !$OMP END PARALLEL DO
...
!    call update_rk3(Zero,Unew, One,U, dt,Uprime)
    do m = 1, ncons
       ...
       do k = lo(3),hi(3)
          do j = lo(2),hi(2)
             do i = lo(1),hi(1)
                Unew(i,j,k,m) = Zero * Unew(i,j,k,m) + One * U(i,j,k,m) + dt * Uprime(i,j,k,m)
             end do
          end do
       end do
       !$OMP END PARALLEL DO
