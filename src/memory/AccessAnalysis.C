	...
	FirstAccessMap_t& firstAccessMap)
{
    ...
    int it_idx=0;
    for(list<SgNode*>::iterator f_it= flatArrayRefList.begin(); f_it != flatArrayRefList.end(); f_it++, it_idx++)
    {
	...
	if(isArrayReference(exp, &arrNameExp, &subscripts)) //no else 
	{
		...
		if(acIt == accList->end())
		{
		    computeDependentLoopVar(this_access, subscripts);
		    computeOffsets(this_access, subscripts);
		    string access_str = getAccessString(subscripts); //previous calls to computeDependentLoopVar and computeOffsets may change subscript
...
int expandClause(SgBinaryOp **op_ptr, vector<SgExpression*> &subscripts, int idxOffset){ //this routine is useful when the lhs and/or rhs of a multiply op is not a varRef
    SgBinaryOp *op= *op_ptr;
    SgExpression* lhs0 = op->get_lhs_operand(); 
    SgExpression* rhs0 = op->get_rhs_operand(); 

    SgExpression *lhs, *rhs;
    if(!isSgVarRefExp(rhs0)) {
	lhs= lhs0; rhs=rhs0;
    }else if(!isSgVarRefExp(lhs0)){
	lhs= rhs0; rhs=lhs0;
    }

    //now rhs is always a binary op
    SgMultiplyOp* newLhs= buildMultiplyOp(copyExpression(lhs), copyExpression(isSgBinaryOp(rhs)->get_lhs_operand()));
    SgMultiplyOp* newRhs= buildMultiplyOp(copyExpression(lhs), copyExpression(isSgBinaryOp(rhs)->get_rhs_operand()));
    SgBinaryOp *newOp;
    if (isSgAddOp(rhs)) newOp = buildAddOp(newLhs, newRhs);
    else if (isSgAddOp(rhs)) newOp = buildAddOp(newLhs, newRhs);
    else if (isSgSubtractOp(rhs)) newOp = buildSubtractOp(newLhs, newRhs);
    else if (isSgSubtractOp(rhs)) newOp = buildSubtractOp(newLhs, newRhs);
    else ROSE_ASSERT(false); //not supported yet

    //if(op == subscripts[idxOffset])   subscripts[idxOffset]= newOp;
    replaceExpression(op, newOp);
...
void AccessAnalysis::normalizeTerms(SgNode *node, bool sign, vector<SgExpression*> &subscripts, int idxOffset){
    if(node==NULL) return;
	...
	if(!isSgVarRefExp(isSgBinaryOp(node)->get_lhs_operand())&&!isSgIntVal(isSgBinaryOp(node)->get_lhs_operand()) || !isSgVarRefExp(isSgBinaryOp(node)->get_rhs_operand())&&!isSgIntVal(isSgBinaryOp(node)->get_rhs_operand()))
	{
		...
		SgBinaryOp** op_ptr = new SgBinaryOp*;
		*op_ptr= isSgBinaryOp(node);
		int returnedSign= expandClause(op_ptr, subscripts, idxOffset);
		normalizeTerms(isSgBinaryOp(*op_ptr)->get_lhs_operand(), sign, subscripts, idxOffset);
		normalizeTerms(isSgBinaryOp(*op_ptr)->get_lhs_operand(), sign && returnedSign, subscripts, idxOffset);
		return;
	    }
	...
	SgExpression* lhs = isSgBinaryOp(node)->get_lhs_operand();
	SgExpression* rhs = isSgBinaryOp(node)->get_rhs_operand();
	if(lhs)normalizeTerms(isSgNode(lhs), sign, subscripts, idxOffset);
	if(rhs)normalizeTerms(isSgNode(rhs), sign, subscripts, idxOffset);
	return;
    }
    if(isSgSubtractOp(node)) {
	SgExpression* lhs = isSgBinaryOp(node)->get_lhs_operand();
	SgExpression* rhs = isSgBinaryOp(node)->get_rhs_operand();
	normalizeTerms(lhs, sign, subscripts, idxOffset);
	normalizeTerms(rhs, sign==true?false:true, subscripts, idxOffset);
	return;
    }
    if(isSgMinusOp(node)){
	SgExpression* operand = isSgUnaryOp(node)->get_operand();
	normalizeTerms(operand, sign==true?false:true, subscripts, idxOffset);
	return;
    }
}
	...
	vector<SgExpression*> subscripts)
{
	...
	else if(varList.size() >= 1)
	{
	    ...
	    normalizeTerms(isSgNode(index), true, subscripts, indexNo);
		...
		if(strideName != ""){
			...
			for(list<clause*>::reverse_iterator it1= clauseList.rbegin(); it1!=clauseList.rend(); it1++){
				...
				if(c1->rhs){
				    ...
				    if(this_access.offset[indexNo] != "" && modified && !skipPlus)
					offset_str << "+";  
				...
				if(!c1->sign){
				    ...
				    if(this_access.offset[indexNo] != "" && modified && !c1->rhs && !skipPlus)
					offset_str << "+";  
				    this_access.offset[indexNo] = offset_str.str() + this_access.offset[indexNo]; 
				    modified=true;
				    skipPlus=true;
	...
	for(it  = subscripts.begin(); it != subscripts.end(); it++, indexNo++)
	{
		...
		for(indexItr= loopIndices->begin(); indexItr!= loopIndices->end(); indexItr++){
			...
			if(foundOneLoopVar){//index variable matches with the loop control variable 
				...
				this_access.dependentLoopVar[indexNo]= "," + this_access.dependentLoopVar[indexNo];
			    this_access.dependentLoopVar[indexNo] = loopName->get_name().str() + this_access.dependentLoopVar[indexNo];
			    this_access.dependentLoopVar[indexNo] = coeff + this_access.dependentLoopVar[indexNo];
