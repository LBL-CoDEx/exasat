...
void FunctionHandler::process(SgNode* node)
{
	...
	bool is_canonical = isCanonicalForLoop (forLoop, &orig_index, & orig_lower, &orig_upper, &orig_stride, NULL, &isIncremental, &isInclusiveUpperBound);
	if(!is_canonical){
		...
		if(isSgLessThanOp(testExp) || isSgLessOrEqualOp(testExp) || isSgGreaterThanOp(testExp) || isSgGreaterOrEqualOp(testExp) || isSgNotEqualOp(testExp))
		{
			...
			SgInitializedName *indexVar = matchedVarRefs[0]->get_symbol()->get_declaration();
