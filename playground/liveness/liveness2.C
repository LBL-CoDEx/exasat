...
void runCurrentFile(vector<string> &argvList, bool debug, bool debug_map) {
  ...
  LivenessAnalysis* liv = new LivenessAnalysis(false,(DefUseAnalysis*)defuse);
  ...
  for (; i!=vars.end();++i) {
    ...
    for (; nodesIt!=nodes.end();++nodesIt) {
      ...
      if(isSgFortranDo(node)){
	SgFortranDo* doloop = isSgFortranDo(node);
	SgBasicBlock* body = doloop->get_body();

	ROSE_ASSERT(isSgNode(body));

	//cout << node->unparseToString() << endl; 
	std::vector<SgInitializedName*> in = liv->getIn(isSgNode(doloop));
	std::vector<SgInitializedName*> out = liv->getOut(isSgNode(doloop));
